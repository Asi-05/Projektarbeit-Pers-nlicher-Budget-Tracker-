...
def editieren():
    ...
    if optionauswahl == '1': #Startet diesen Block nur wenn 1 gewählt wurde
        ...
        while True: #Beginnt eine neue Schleife für die Einnahme oder Ausgabe
            AoE_auswahl = input('\nBitte Einnahme oder Ausgabe angeben:').strip() #Fordert den User aus Einnahme oder Ausgabe einzugeben und das .strip entfernt Leerzeichen
            if AoE_auswahl == 'Einnahme': #Wird ausgeführt wenn der User Einnahmen eingibt
        ...
        for row in daten: #Beginnt eine Schleife, die jede Zeile (jeden Eintrag), in der Liste daten durchläuft
            if (float(row['betrag']) == betrag_suche and #Prüft ob Betrag der akutellen Zeile mit dem gesuchten übereinstimmt
                ...
                bestaetigung = input('Diesen Eintrag löschen? (ja/nein):').lower() #fragt den User ab und wertet es als Kleinschreibung
                if bestaetigung=='ja':
        ...
        for row in daten: #Durchläuft jeden Eintrag
            if ( #Prüft ob der eintrag mit allen Suchkriterien übereinstimmt
                ...
                print(f'\nGefundener Eintrag: {row['datum']} | {row['betrag']} | {row['typ']} | {row['kategorie']}') #Gibt den gefunden Eintrag aus
                bestaetigung = input('Diesen Eintrag bearbeiten? (Ja/Nein):').lower() #Fordert Eingabe und wandelt es in kleinschreibung um 
                if bestaetigung=='ja': #Wenn die Bearbeitung bestätigt wird:
                    neuer_typ= input('Einnahme oder Ausgabe:') #Fragt nach Typ
                    neuer_betrag= input('Neuer Betrag: ') #Fragt nach Betrag
                    neues_datum= input('Neues Datum (TT.MM.JJJ):') #Fragt neues Datum
